...
function setGLTFMaterial ( self, modelEntity , obj , cubeTex , cameraTexture ){

    // let self = this;

    let objj = modelEntity.getObject3D('mesh');

    if ( obj.materialAttr && obj.materialAttr.materials){
...
export function loadGLTFModel( scene3DRoot , obj, position, rotation, scale, quaternion ) {
    ...
    let cubeTex = arController.cubeTex;
    let cameraTexture = arController.cameraTexture;
    ...
    let pModel = new Promise( function( modelResolve ){
        ...
        UrlExistsFetch( obj.res_url ).then( retStatus =>{
            ...
            if ( retStatus == true ){
                ...
                console.log(' __loadGLTFModel__:  ' , obj )
                ...
                ( obj.sub_type != 'glb' && obj.sub_type != 'gltf'  && obj.sub_type != 'gltf_sketchFab'  && obj.sub_type != 'gltf_sketchfab' &&
                  obj.sub_type != 'gltf_poly'
                    ...
                    console.log(" _loadGLTFModel_: obj not support ", obj );
                    modelResolve( -1 );
                    return;
                }
                ...
                modelEntity.addEventListener("model-loaded", function(evt){ // model-loaded  / object3dset
                    ...
                    if ( evt.target ==  evt.currentTarget ){
                        ...
                        if ( obj.generalAttr.obj_parent_id ){
                            ...
                            modelEntity.object3D.obj_parent_id = obj.generalAttr.obj_parent_id;
                            ...
                            setARTransform ( modelEntity, dp, rotation, scale, quaternion );
                            ...
                            console.log('_loadGLTFModel_: loaded: no parent prs=' , modelEntity.object3D.position , modelEntity.object3D.rotation, modelEntity.object3D.scale  );
                        ...
                        if ( modelEntity.object3D ){
                            ...
                            console.log("GLTFModelModule.js _loadGLTFModel, obj.materialAttr=", obj.materialAttr)

                            if (obj.materialAttr.materials && obj.materialAttr.materials.length > 0){

                                console.log("obj.materialAttr.materials", obj.materialAttr.materials)
                                //// 檢查是不是 "舊版(v3.4)材質球"
                                let isV340material = false
                                obj.materialAttr.materials.forEach(element => {
                                    let material_idx = element.material_idx;
                                    if(!material_idx){
                                    //// 表示它是舊版(v3.4)材質球 但是專案升到新版(v3.5.0.0)
                                        isV340material = true
                                    }
                                ...
                                if(isV340material){
                                    //// 表示它是舊版(v3.4)材質球 但是專案升到新版(v3.5.0.0)
                                    setGLTFMaterial( self, modelEntity , obj , cubeTex , cameraTexture );
                                }
                                else {
                            ...
                            if ( editor_ver && editor_ver.v0 == 3 && 
                                ...
                                if ( obj.generalAttr && obj.generalAttr.logic == true ){
                                    pm.resetProperty = function(){
                                        ...
                                        if ( typeof( obj.generalAttr.active ) == 'boolean' ){
                                            ...
                                            modelEntity.setAttribute("visible", obj.generalAttr.active );
                                        }
                                    }
                                }
                            }
                            ...
                            if (Array.isArray(evt.detail.model.animations)){
                                if ( evt.detail.model.animations.length>0 && !modelEntity.getAttribute("animation-mixer") ){
                                    console.log("_loadGLTFModel_: the model with animation but no animation-mixer, probabily older version of editor ");
                        ...
                        // console.log(" _loadGLTFModel_ ", obj.res_name, modelEntity.object3D.children );
                    }
                });
                ...
                if(obj.generalAttr.obj_parent_id){
                    ...
                    scene3DRoot.appendChild( modelEntity );

                }
                
            }else{

                console.log("_loadGLTFModel_: , obj url not exist ", obj );
            ...
            objj.traverse(node => {
                ...
                if (node.material) {
                    ...
                    // if (node.material.name == obj.materialAttr.materials[i].name) {
                        ...
                        // console.log('XRFunc.js: _loadGLTFModel: obj.materialAttr.materials',obj.materialAttr.materials);
                        // console.log('XRFunc.js: _loadGLTFModel: standard node.material',node.material);
            ...
            for (let i = 0; i < obj.materialAttr.materials.length;i++){
                ...
                for(let j = 0; j < material.properties.length; j++){
                    ...
                    let pTexture = new Promise(function(textureResolve){
                        ...
                        if(texture_url){
                            // UrlExistsFetch( texture_url , function( retStatus ){
                            UrlExistsFetch( texture_url ).then( retStatus =>{
...
function loadMaterial_v340(self, modelEntity, obj, objj, i){
    ...
    // console.log("v340 ____loadGLTFModel obj.materialAttr=", obj.materialAttr)

    let materialIndex =  checkGLTFMaterialIndex_v340( modelEntity, obj.materialAttr.materials[i] )
...
function checkGLTFMaterialIndex_v340( target, material ){
...
export function loadWebViewNotSupport(scene3DRoot, obj, index, sceneObjNum, position, rotation, scale, quaternion){
    //// 這裡this是vrController
    const self = this;
    
    let dpi = self.gcssTargets.dpi[ scene3DRoot.GCSSID] ; 
    // console.log("_loadWebViewNotSupport dpi", dpi)
    let GCSSWidth = self.gcssTargets.width[ scene3DRoot.GCSSID] ; 
    let GCSSHeight = self.gcssTargets.height[ scene3DRoot.GCSSID] ; 

    // console.log("_loadWebViewNotSupport", obj, index, sceneObjNum, position, rotation, scale)
    ...
    ytEntity.addEventListener("loaded", e => {
        ...
        console.log("_loadWebViewNotSupport ytEntity.object3D", ytEntity.object3D)
        ...
        if ( obj.generalAttr.obj_parent_id ){
            ...
            ytEntity.object3D.obj_parent_id = obj.generalAttr.obj_parent_id;
            ...
            setARTransform( ytEntity, dp, rotation, scale.multiplyScalar(scaleCoefficient), quaternion );
            ...
            //// 第一層物件必須放置於辨識圖中央
            ytEntity.object3D.position.add( new THREE.Vector3( GCSSWidth*25.4/dpi/2 , GCSSHeight*25.4/dpi/2 , 0 ) );
            //// 第一層物件必須垂直於辨識圖表面
            ytEntity.object3D.rotation.x += Math.PI*90/180;

            //// 20240620 renhaohsu想問參數如何決定 (因為底下需要的平面可能也要調)

            // console.log('three.js: _loadAframeVideo: loaded: no parent prs=' , ytEntity.object3D.position , videoPlane.object3D.rotation, videoPlane.object3D.scale  );

        }



        console.log("_loadWebViewNotSupport ytEntity loaded", )
        ...
        const planeCoefficient = 25
        plane.scale.set( planeCoefficient, planeCoefficient , 1 );
        ...
        console.log("_loadWebViewNotSupport ytEntity loaded", plane)
        ...
        //// 微調一下小平面的縮放
        const paddingCoeficient = planeCoefficient*0.95
        paddingPlane.scale.set(paddingCoeficient, paddingCoeficient, 1)
        console.log("_loadWebViewNotSupport ytEntity loaded", paddingPlane)
        ...
        let pText = self.loadText3D( scene3DRoot, tempTextObj , new THREE.Vector3(0,0,0), new THREE.Vector3(0,0,0), new THREE.Vector3(1,1,1), new THREE.Quaternion(), true )
    ...
    if(obj.generalAttr.obj_parent_id){
        ...
        scene3DRoot.appendChild( ytEntity )
        
        console.log("_loadWebViewNotSupport ytEntity loaded", ytEntity)
    }
}

//// WebView 物件 2D
////   (只是暫時物件，先不加入promise all的行列)
export function loadWebViewNotSupport2D(scene2DRoot, obj, index, sceneObjNum, position, rotation, scale){
    ...
    rootObject["makarType"] = 'WebView2D';
    ...
    let pText = self.loadText2D( scene2DRoot, tempTextObj , index, sceneObjNum, null, null, null, true)  
