...
export function loadVideo3D( scene3DRoot, obj, position, rotation, scale , quaternion ) {	
    ...
    let pVideo = new Promise( function( videoResolve ){
        ...
        UrlExistsFetch( obj.res_url ).then( retStatus =>{
            ...
            if ( retStatus == true ){
                ...
                let projIndex = self.sceneTargetList[  scene3DRoot.GCSSID ].projIndex ;
                ...
                mp4Video.setAttribute("id", obj.generalAttr.obj_id+"_"+obj.res_id );
                ...
                mp4Video.onloadedmetadata = function() {
                    ...
                    if ( editor_ver && editor_ver.v0 == 3 && 
                        ...
                        if (transparentVideo && transparentBehav){
                            ...
                            videoPlane.setAttribute( "material", "side:double; opacity: 1.0; transparent: true; " ); 
                        }

                        //// 物件 ID
                        videoPlane.setAttribute( "id", obj.generalAttr.obj_id );//// fei add 
                        videoPlane.setAttribute("src", "#"+obj.generalAttr.obj_id+"_"+obj.res_id ); //  
                    ...
                    // videoPlane.addEventListener("object3dset", function(evt){
                        ...
                        if (evt.target == evt.currentTarget){
                            ...
                            if ( obj.generalAttr.obj_parent_id ){
                                ...
                                videoPlane.object3D.obj_parent_id = obj.generalAttr.obj_parent_id;
                                ...
                                setARTransform( videoPlane, dp, rotation, scale, quaternion );
                            ...
                            if ( obj.generalAttr && obj.generalAttr.logic ){
                                pm.resetProperty = function(){
                                    ...
                                    if ( typeof( obj.generalAttr.active ) == 'boolean' ){
                                        ...
                                        videoPlane.setAttribute("visible", obj.generalAttr.active );
                                        if ( obj.generalAttr.active == false ){
                    ...
                    if(obj.generalAttr.obj_parent_id){
                        ...
                        scene3DRoot.appendChild( videoPlane );
                ...
                if (window.Browser.mobile == true && window.Browser.name != "safari" ){
                    ...
                    if(obj.typeAttr.is_loop){
                        ...
                        mp4Video.addEventListener('ended', function() { 
                            ...
                            if( document.getElementsByTagName('a-video').length + window.arController.makarObjects2D.filter( o=>o.makarType == "video2D").length == 1 ){
                                console.log("只剩一個影片了 播放它")
                                mp4Video.currentTime=0.1;
                                mp4Video.muted = false
                                mp4Video.play(); 
                            } else {
                                ...
                                mp4Video.muted = true
                                mp4Video.play()
                                UnMutedAndPlayAllVisibleVideo(mp4Video)
                            }
                        }, false);
                    }
                }
...
function dealVideoMuted( video, isSafari=false ){      
    console.log("dealVideoMuted isSafari", isSafari)  

    //// html確認面板"允許聲音播放"
    let clickToPlayAudio = document.getElementById("clickToPlayAudio");

    //// 註: 看起來會運作的其實是 ARWrapper.js 的 dealVideoMuted 不過這裡還是補一下幾乎一樣的東西
    clickToPlayAudio.onclick = function(){
        
        console.log("dealVideoMuted clickToPlayAudio isSafari", isSafari)  
...
export function UnMutedAndPlayAllVisibleVideo( targetVideo_in ) {
    ...
    console.log("XRFunc.js: _UnMutedAndPlayAllVideo: aVideos.length=", aVideos.length );

    //// 取得「所有影片」  2D
    let videos2D = window.arController.makarObjects2D
    ...
    if ( targetVideo ){
        ...
        for ( let i = 0; i < aVideos.length; i++ ){
            ...
            if ( videoPlane == targetVideo ){
                ...
                videoPlane.object3D.traverseAncestors( function(parent) {
                    if (parent.type != "Scene"){
                        // console.log("XRFunc.js: arController: _UnMutedAndPlayAllVideo,: traverseAncestors: not Scene parent=", parent );
                        if (parent.visible == false){
                            parentVisible = false;
                        }
                    } else {

                        console.log("XRFunc.js: _UnMutedAndPlayAllVideo: videoPlane =", i, parentVisible , videoPlane.object3D.visible, videoPlane );
        ...
        targetVideo2D.traverseAncestors( function(parent) {
            if (parent.type != "Scene"){
                // console.log("VRFunc.js: arController: _UnMutedAndPlayAllVideo,: traverseAncestors: not Scene parent=", parent );
                if (parent.visible == false){
        ...
        for ( let i = 0; i < aVideos.length; i++ ){
            ...
            if( !isVideoPlaying(mp4Video) ){ continue; }
                ...
                if (parent.type != "Scene"){
                    ...
                    if (parentVisible == true && videoPlane.object3D.visible == true && !videoPlane.blockly && mp4Video.volume!=0 ){
                        ...
                        if ( setVideoUnMuted == false ){
                            console.log("XRFunc.js: _UnMutedAndPlayAllVideo: all parent visible true , _setVideoUnMuted false ", videoPlane.object3D.name4debug );
                            ...
                            console.log("XRFunc.js: _UnMutedAndPlayAllVideo: all parent visible true, _setVideoUnMuted true", videoPlane.object3D );
...
export function loadVideo2D( scene2DRoot, obj, index, sceneObjNum, position, rotation, scale ){
    let self = this;
                            ...
                            if (self.makarObjects2D[i].obj_id == obj.generalAttr.obj_parent_id){
                                ...
                                rootObject.traverseAncestors( function(parent) {                                    
                                    ...
                                    if (parentVisible == true && rootObject.visible == true){
                                        console.log("VideoModule.js: _loadVideo2D: traverseAncestors: all parent visible true=", rootObject , videoBehavRef );
                                        ...
                                        console.log("VideoModule.js: _loadVideo2D: traverseAncestors: not all parent visible true=", parentVisible, videoPlane.object3D.visible, videoBehavRef );
                                        //// rootObject.visible = false; 
                                        mp4Video.muted = false;
                                        mp4Video.autoplay = false;
                                        mp4Video.pause();
...
export function loadYouTubeNotSupport(scene3DRoot, obj, index, sceneObjNum, position, rotation, scale, quaternion){
    //// 這裡this是vrController
    const self = this;
    
    let dpi = self.gcssTargets.dpi[ scene3DRoot.GCSSID] ; 
    // console.log("_loadYoutubeNotSupport dpi", dpi)
    let GCSSWidth = self.gcssTargets.width[ scene3DRoot.GCSSID] ; 
    let GCSSHeight = self.gcssTargets.height[ scene3DRoot.GCSSID] ; 

    // console.log("_loadYoutubeNotSupport", obj, index, sceneObjNum, position, rotation, scale)
    ...
    ytEntity.addEventListener("loaded", e => {
        ...
        console.log("_loadYoutubeNotSupport ytEntity.object3D", ytEntity.object3D)
        ...
        if ( obj.generalAttr.obj_parent_id ){
            ...
            setARTransform( ytEntity, dp, rotation, scale, quaternion );
        ...
        console.log("_loadYoutubeNotSupport ytEntity loaded", )
        ...
        plane.scale.set( scaleCoefficient, scaleCoefficient , 1 );
        ...
        console.log("_loadYoutubeNotSupport ytEntity loaded", plane)
        ...
        //// 微調一下小平面的縮放
        const paddingCoeficient = 25*0.95
        paddingPlane.scale.set(paddingCoeficient, paddingCoeficient, 1)
        console.log("_loadYoutubeNotSupport ytEntity loaded", paddingPlane)
    ...
    if(obj.generalAttr.obj_parent_id){
        ...
        console.log("_loadYoutubeNotSupport ytEntity loaded", ytEntity)
    }
}

//// YT影片物件 2D
////   (只是暫時物件，先不加入promise all的行列)
export function loadYouTubeNotSupport2D(scene2DRoot, obj, index, sceneObjNum, position, rotation, scale){
