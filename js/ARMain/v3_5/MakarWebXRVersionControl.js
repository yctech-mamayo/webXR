...
class VerionControl {
    ...
    getObjTransform( sceneData , scene_obj ){
        ...
        //// 3.5.0
        //// 這邊註記一下， AR 流程目前 設定場景物件的 transform 是直接使用 quaternion ，沒有使用 rotation。
        //// 但是 VR/XR 還是有使用 rotation，所以這邊依然保留紀錄 rotation。
        ...
        if ( v0 == 3 && v1 >= 5 ){
            ...
            if ( scene_obj.generalAttr && scene_obj.generalAttr.obj_type == '2d'  ){
                    ...
                    scene_obj.transformAttr.rect_transform[0] )
                {
                    ...
                    //// 編輯器取得的 quaternion 順序為 XYZW 
                    let quaternionArray = scene_obj.transformAttr.rect_transform[0].rotation.split(",").map( x => Number(x) ) 
                    transform.quaternion = new THREE.Quaternion( quaternionArray[0], quaternionArray[1], quaternionArray[2], quaternionArray[3] )
    

                    //// 這邊註記一下， 所有的 THREE 相關物件的 rotation 設定，order = XYZ
                    //// 但是 Aframe 所創建出的 a-entity 物件， rotation setAttribute 後，都是 YXZ。
                    //// 故這邊要設定的旋轉要作用在 a-entity 的 object3D上，由 quaternion 取得 Euler 順序設定為 YXZ 
                    //// 後面在需要設定「場景物件」時，調整 y , z 的正負號 （ 左右手 ）

                    // let eulerAngle = new THREE.Euler().setFromQuaternion( transform.quaternion, "XYZ");
                ...
                if ( scene_obj.transformAttr && scene_obj.transformAttr.transform ){
                    ...
                    console.warn(' _getObjTransform_ : v3.5.0.0 type 3d error ' , scene_obj );
                }
