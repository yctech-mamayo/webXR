...
import AddObjectToArController from "./AddObjectToArController.js"
import { isPromise, getRandomInt, checkDefaultObj } from "./utility.js"
import { setTransform } from './arObjModules/setTransform.js'

import { verionControl as VC } from "./MakarWebXRVersionControl.js";

////  AddObjectToArController 是一個 "與ARController互動" 的Class。 Quiz繼承它 表示Quiz可以和ARController互動
class Quiz extends AddObjectToArController {
    ...
    constructor(obj){
        ...
        //// 從ARController取得資料: 在 updateRecordModule() 時需要post到後端API的資料
        if(window.arController){
            this.currentProjData.user_id         = window.arController.currentProjData.user_id
            this.currentProjData.proj_id         = window.arController.currentProjData.proj_id
            this.currentProjData.head_pic        = window.arController.currentProjData.head_pic
            this.currentProjData.loc             = window.arController.currentProjData.loc
            this.currentProjData.module_type     = window.arController.currentProjData.module_type
            this.currentProjData.name            = window.arController.currentProjData.name
            this.currentProjData.proj_cover_urls = window.arController.currentProjData.proj_cover_urls
            this.currentProjData.proj_name       = window.arController.currentProjData.proj_name
            this.currentProjData.shared_id       = window.arController.currentProjData.shared_id
            this.currentProjData.snapshot_url    = window.arController.currentProjData.snapshot_url
        }
    ...
    //// ----------------
    //// 為了與arController互動 先覆寫: "把物件加入到 場景 或 arController 常用到的函式們" 

    addTextToScene( jsonObj, position, rotation, scale, quaternion, index=0, sceneObjNum=1 ){
        //// 把文字物件加入場景
        if( jsonObj.generalAttr.obj_type == "3d" && this.checkScene3DRoot() ){
            return window.arController.loadText3D( this.scene3DRoot, jsonObj, position, rotation, scale, quaternion )
        } else if( jsonObj.generalAttr.obj_type == "2d" && this.checkScene2DRoot() ){
            return window.arController.loadText2D( this.scene2DRoot, jsonObj, index, sceneObjNum, position, rotation, scale )
        } else {
            console.log("_Quiz _loadQuestion: There shall not be any type except for 2d3d")
        }
    }
    
    addTextureToScene( jsonObj, position, rotation, scale, quaternion, index=0, sceneObjNum=1 ){
        //// 把圖片物件加入場景
        if( jsonObj.generalAttr.obj_type == "3d" && this.checkScene3DRoot() ){
            return window.arController.loadTexture3D( this.scene3DRoot, jsonObj, position, rotation, scale, quaternion )
        } else if( jsonObj.generalAttr.obj_type == "2d" && this.checkScene2DRoot() ){
            return window.arController.loadTexture2D( this.scene2DRoot, jsonObj, index, sceneObjNum, position, rotation, scale )
        } else {
            console.log("_Quiz _loadQuestion: There shall not be any type except for 2d3d")
        }    
    }

    addGLTFModelToScene( jsonObj, position, rotation, scale, quaternion ){
        //// 把模型物件加入場景
        if( jsonObj.generalAttr.obj_type == "3d" && this.checkScene3DRoot() ){
            return window.arController.loadGLTFModel( this.scene3DRoot, jsonObj, position, rotation, scale, quaternion )  
        } else {
            console.log("%c quiz loadGLTFModel: quiz currently does not support GLTFModel in 2D scene.", "color:tomato;")
        }
    }

    addAudioToScene( jsonObj, position, rotation, scale, quaternion ){
        //// 把聲音物件加入場景
        if( jsonObj.generalAttr.obj_type == "3d" && this.checkScene3DRoot() ){
            return window.arController.loadAudio( this.scene3DRoot, jsonObj, position, rotation, scale, quaternion )
        } else {
            console.log("%c quiz loadAudio: quiz currently does not support Audio in 2D scene.", "color:tomato;")
        }
    }

    addVideoToScene( jsonObj, position, rotation, scale, quaternion, index=0, sceneObjNum=1 ){
        //// 把影片物件加入場景
        if( jsonObj.generalAttr.obj_type == "3d" && this.checkScene3DRoot() ){
            return window.arController.loadVideo3D( this.scene3DRoot, jsonObj, position, rotation, scale, quaternion )
        } else if( jsonObj.generalAttr.obj_type == "2d" && this.checkScene2DRoot() ){
            return window.arController.loadVideo2D( this.scene2DRoot, jsonObj, index, sceneObjNum, position, rotation, scale )
        } else {
            console.log("_Quiz _loadQuestion: There shall not be any type except for 2d3d")
        }
    }
    ...
    pushToMakarObjects( makarObject ){
        //// 把3D物件加入到 _makarObjects 陣列
        if(window.arController.makarObjects && Array.isArray(window.arController.makarObjects)) window.arController.makarObjects.push( makarObject );
        else console.warn("window.arController.makarObjects does not exist")
    }

    pushToMakarObjects2d( makarObject ){
        //// 把3D物件加入到 _makarObjects2D 陣列
        if(window.arController.makarObjects2D && Array.isArray(window.arController.makarObjects2D)) window.arController.makarObjects2D.push( makarObject );
        else console.warn("window.arController.makarObjects does not exist")
    }

    setTransform(base, circlePos, circleRot, circleScale){
        //// 把物件放到對應的場景位置
        // window.arController.setTransform(base, circlePos, circleRot, circleScale);
        setTransform(base, circlePos, circleRot, circleScale);
    }

    setARTransform ( obj, position, rotation, scale, quaternion  ){
        //// AR才有這個method (應該是)把物件放到對應的場景位置
        window.arController.setARTransform( obj, position, rotation, scale, quaternion )
    }

    appendToArScene(quizEntity, jsonObj){
        if(this.checkScene3DRoot()){
    ...
    appendToArScene2d(quizEntity, jsonObj){
        if(this.checkScene2DRoot()) {
            ...
            if(jsonObj.generalAttr.obj_parent_id){
                ...
                let timeoutID = setInterval( function () {
                    let parent = null
                    for (let i = 0; i < window.arController.makarObjects2D.length; i++ ){
                        if ( window.arController.makarObjects2D[i].obj_id == jsonObj.generalAttr.obj_parent_id  ){
                            parent = window.arController.makarObjects2D[i];
                        }
                    }
                ...
                this.scene2DRoot.add(quizEntity.object3D)
            }
    ...
    checkScene3DRoot(){
        if(this.scene3DRoot){
            ...
            console.warn("scene3DRoot does not exist, or quiz is not loaded. scene3DRoot=", this.scene3DRoot)
            return false
        }
    }
    ...
    checkScene2DRoot(){
        if(this.scene2DRoot){
            ...
            console.warn("scene2DRoot does not exist, or quiz is not loaded. scene2DRoot=", this.scene2DRoot)
            return false
        }
    }

    //// ---------------- 其他會用到 arController 的細項們
    clearObject(){
        //// 把物件從 arController.makarObject 清除、從頁面上移除
        if(this.obj_type == "3d"){
            ...
            temp.forEach( item => {
                ...
                for (let i = 0; i < window.arController.makarObjects.length; i++ ){
                    if (window.arController.makarObjects[i].id == item.id){
                        let makarObject = window.arController.makarObjects[i];
                        ...
                        makarObject.remove();
                        window.arController.makarObjects.splice(i,1);
                    }
                }
            });
            ...
            childrens.forEach( child => {
                arController.makarObjects2D = arController.makarObjects2D.filter(item => item !== child)
            })
            childrens.length = 0
    ...
    setTypesFromUserRes(obj){
        //// res_url and main_type no longer exist in ver. 3.5's ARSceneResult
    ...
    //// 載入 quiz
    load( scene3DRoot, scene2DRoot, obj, transformData ) {

	    //[start-20240124-renhaohsu-modify]//
        //// AR的場景本身，相當於 vr,xr的 vrScene,xrScene
        this.scene3DRoot = scene3DRoot;
        this.scene2DRoot = scene2DRoot;

        const position = transformData.position
        const rotation = transformData.rotation
        const scale = transformData.scale
        const quaternion = transformData.quaternion

        //// quiz物件 對辨識圖平移
        const dpi = transformData.panByScene3DRoot.dpi
        const GCSSWidth = transformData.panByScene3DRoot.GCSSWidth
        const GCSSHeight = transformData.panByScene3DRoot.GCSSHeight
        ...
        quizEntity.object3D.makarType = "quiz"
        ...
        // //// 此物件是直接掛載在「基礎辨識圖」，所以位置上要平移到中央，旋轉上 x 要多加 90 度
        // let py = dp.y;
        // let pz = dp.z;
        // dp.y = pz;
        // dp.z = py;
        ...
        if(obj.generalAttr.obj_type == "3d"){
            ...
            quizEntity.addEventListener( 'loaded' , function(){
                ...
                if ( obj.generalAttr.obj_parent_id ){
                    ...
                    quizEntity.object3D.obj_parent_id = obj.generalAttr.obj_parent_id;
                    ...
                    switch (window.serverVersion){
                        case "2.0.0":
                            scale.multiplyScalar( 1 );
                            _dp.addScaledVector( position, 1*100*25.4/dpi ); // 100 is the factor of unity
                            break;

                        case "3.0.0":
                            //// renaohsu依照經驗法則 子物件的scale改回1 不明白為何這樣看上去是正確的
                            // scale.multiplyScalar( 2 );
                            scale.multiplyScalar( 1 );
                            // _dp.addScaledVector( position, 2*100*25.4/dpi ); // 2*100 is the factor of unity
                            _dp.addScaledVector( position, 1*100*25.4/dpi );
                            break;
                        default:
                            console.log(" _loadGLTFModel_: serverVersion version wrong", serverVersion);
                    }
                    ...
                    _dp.y = pz;
                    _dp.z = py;
                    window.arController.setARTransform( quizEntity, _dp, rotation, scale, quaternion );
                    ...
                    dp.y = pz;
                    dp.z = py;
                    window.arController.setARTransform( quizEntity, dp, rotation, scale, quaternion );

                    //// 第一層物件必須放置於辨識圖中央
                    quizEntity.object3D.position.add( new THREE.Vector3( GCSSWidth*25.4/dpi/2 , GCSSHeight*25.4/dpi/2 , 0 ) );
                    //// 第一層物件必須垂直於辨識圖表面
                    quizEntity.object3D.rotation.x += Math.PI*90/180;
                    console.log('_loadGLTFModel_: loaded: no parent prs=' , quizEntity.object3D.position , quizEntity.object3D.rotation, quizEntity.object3D.scale  );
                }
            ...
            let scaleRatioXY = window.arController.scaleRatioXY
        ...
        if(!obj.typeAttr.module.display_order_list){
            //// 已經在 loadSceneObject 時 就先擋掉，理論上走不到這裡
        ...
        if(first_question.trigger_type){
            if(first_question.trigger_type == "Target"){
                console.log("%c _quiz _load _first_question 辨識圖觸發 目前web暫時跳過", "color: Khaki")

                if(obj.generalAttr.obj_type == "3d"){
                    // this.setARTransform(quizEntity, dp, rotation, scale, quaternion);
                    this.appendToArScene(quizEntity, obj)
                } else {
                    //// 2D setransform 會正確 ?!  待測試
                    this.setTransform(quizEntity, position, rotation, scale);
                    this.appendToArScene2d(quizEntity, obj)
                }
        ...
        if ( first_question.questions_json && Array.isArray( first_question.questions_json ) ){
            for(let i=0; i<first_question.questions_json.length; i++){
                ...
                let pQuestion = this.loadQuestion(first_question.questions_json[i])                  
    ...
    loadQuestion(question_json){
        ...
        let objTranform = VC.getObjTransform( window.arController.scenesData , question_json );
        ...
        if(question_json.generalAttr.obj_type == "3d"){
            ...
            let selectedResolutionIndex = window.arController.selectedResolutionIndex
            if( !window.arController.selectedResolutionIndex ) selectedResolutionIndex = 0;
        ...
        switch(main_type){
            case "text":                        
                pQuestion = this.addTextToScene( obj, position, rotation, scale, quaternion )
                break;
            case "image":
                pQuestion = this.addTextureToScene( obj, position, rotation, scale, quaternion, 0, 1 )
                break;
            case "video":
                pQuestion = this.addVideoToScene( obj, position, rotation, scale, quaternion )
                break;
            case "model":   
                pQuestion = this.addGLTFModelToScene( obj, position, rotation, scale, quaternion )            
                break;
            case "audio":
                pQuestion = this.addAudioToScene( obj, position, rotation, scale, quaternion )
                break;
        }
        return pQuestion
    }
    ...
    loadOption(option_json){
        ...
        //// 為每個物件設定 position、quaternion、scale
        let objTranform = VC.getObjTransform( window.arController.scenesData , option_json );
        let position = objTranform.position;
        let rotation = objTranform.rotation;
        let scale = objTranform.scale;
        let quaternion = objTranform.quaternion;
        ...
        obj.main_type = "button"  //// 雖已改用 behav 來觸發，但Quiz.js仍有一些需要用 main_type=="button" 來判斷的功能
        ...
        switch(sub_type){
            case "txt":
                pOption = this.addTextToScene( obj, position, rotation, scale, quaternion )
                break;
            ...
            case "jpeg":
            case "png":
                pOption = this.addTextureToScene( obj, position, rotation, scale, quaternion )
                break;
    ...
    //// 3d 文字多選 : 在文字選項左側加上圓圈
    multiOptionTextCircle3d(optionObject, circlePos, circleRot, circleScale, scaleCoefficient=0.7){
    ...
    //// 2d 文字多選 : 在文字選項左側加上圓圈
    multiOptionTextCircle2d(optionObject, circlePos, circleRot, circleScale, scaleCoefficient=0.7){
    ...
    //// 3d 圖片多選 : 在圖片選項下方加上圓圈
    multiOptionImageCircle3d(optionObject, circlePos, circleRot, circleScale, scaleCoefficient=50){
        //// source: v3.4 artoolkit.three.js _loadQuiz
        ...
        // circlePos.y = circlePos.y * scaleCoefficient
        ...
        base.setAttribute( "material", "shader:flat; side:double; opacity: 1.0; transparent: true; color:#3C3C3C; depthWrite:false" );
        this.setTransform(base, circlePos, circleRot, circleScale);
        // this.setARTransform(base, circlePos, circleRot, circleScale, newQuaternion);
        optionObject.appendChild(base);
        ...
        circle.setAttribute( "material", "shader:flat; side:double; opacity: 1.0; transparent: true; color:#7B7B7B; depthWrite:false" ); 
        this.setTransform(circle, circlePos, circleRot, circleScale);
        // this.setARTransform(circle, circlePos, circleRot, circleScale, newQuaternion);
        optionObject.appendChild(circle);
        ...
        circleScale.multiply( new THREE.Vector3( 0.7,0.7,0.7 ) );
        this.setTransform(circle2, circlePos, circleRot, circleScale);
        // this.setARTransform(circle2, circlePos, circleRot, circleScale, newQuaternion);
        optionObject.appendChild(circle2);
	...
	quizOption(target){
        ...
        if( window.aQuizAR.checkIs2DPictureShowing() ) return;
    ...
    nextQuestion(){
        ...
        if (this.module.currentIndex < this.module.json.display_order_list.length){
            ...
            if(next_question.trigger_type){
                if(next_question.trigger_type == "Target"){
                    console.log("%c _quiz _nextQuestion 辨識圖觸發 目前web暫時跳過", "color: Khaki")
                    this.nextQuestion()
                    return
                }
            }
    ...
    saveQuizStatus(){				
        //// 沒有下一題，把『觸控』開回來
        // arController.cursorEntity.setAttribute('cursor', "fuse: true; fuseTimeout: 5" );

        //// 記錄為已遊玩結束，讓AR掃到辨識圖時能不再跳出html ui  (在aQuizAR.checkIfQuizOpenDirectly那邊判斷)
        this.isPlayed = true;
        ...
        console.log("Quiz.js: quiz end , this.module.record = " , this.module.record  );
        ...
        net.getRecordModule( playing_user , this.currentProjData.proj_id ).then( result => {
            ...
            if(quizProjectModuleRecord){
                //// 記下quiz.obj_id 使得之後可以GET"同專案下的哪個Quiz已遊玩過"
...
//// 嘗試 design pattern: flyweight pattern (享元模式)
...
class QuizFactory {
    ...
    //// 記錄當前辨識圖 才能判斷在哪個AR場景  並將問答的物件加到場景下
    scene3DRoot;
    scene2DRoot;
    ...
    //// 檢查 "這個AR場景下" 是否有Quiz正在遊玩
    checkIsAnyQuizPlaying(){
        // console.log("checkIsAnyQuizPlaying", this.scene3DRoot)
        return this.loadedQuizzes.some( q => { return q.isPlaying == true && q.scene3DRoot==this.scene3DRoot } )
    }

    //// 清除正在控管的quiz物件們。搭配 aRController loadScene()使用，讓場景切換時能清空 aQuizAR
    clear(){
        // this.quizData = [];
        // this.loadedQuizzes.forEach(q=>{
        //     q.isPlaying = false
        // })
        // this.loadedQuizzes = [];
        ...
        document.getElementById("QuizStartButton").removeEventListener('click', this.startQuizFunc)
    }
    ...
    saveQuizDataToMDB(quiz_obj_id, user_id, project_id, device_id){   
        ...
        this.getQuizProjectFromMDB(project_id).then( getProjRet => {
            ...
            if ( Object.keys(getProjRet).length == 0 ) {    
                ...
                projInfo.quizzes[quiz_obj_id] = quizzesInfo
                // console.log("pointCard.js: _setPointCardProject projInfo.pointCards[pointCardInfo.obj_id]=" , projInfo.pointCards[pointCardInfo.obj_id]);
                // resolve (parent.mdb.setPointCardProject( projInfo ))
            ...
            // if ( getProjRet ){
            //     //// 如果目前專案下 mdb已經有其他quiz物件有遊玩過的資料                
            //     let current_quiz_obj_ids = []
            //     if( Array.isArray(getProjRet.quiz_obj_ids) ){ 
                    
            //         current_quiz_obj_ids = getProjRet.quiz_obj_ids

            //         //// 當有複數個quiz時 檢查此quiz的obj_id是否已記錄。  若未記錄，則記錄
            //         const foundQuizRecord = getProjRet.quiz_obj_ids.find( quizRecordMdb => quizRecordMdb == quiz_obj_id)
            //         if(!foundQuizRecord){
            //             current_quiz_obj_ids.push(quiz_obj_id)    
            //         }

            //     } else {
            //         current_quiz_obj_ids = [ quiz_obj_id ]
            //     }

            //     quizzes.quiz_obj_ids = current_quiz_obj_ids
            //     let p = this.putQuizProjectToMDB ( projInfo )    
            //     // p.then(data => console.log("____________mdb put!   data", data))                                  
            
            // } else {
            //     //// 如果目前專案下 沒有遊玩過quiz 直接存一筆資料到mdb
            //     quizzes.quiz_obj_ids = [ quiz_obj_id ]                
            //     this.putQuizProjectToMDB ( projInfo )
            // }
    ...
    loadQuiz( scene3DRoot, scene2DRoot, jsonObj, transformData, proj_id, login_id, UIs, worldContent, languageType ){

        //// 為了之後檢查當前場景是否已經有開啟著的Quiz
        this.scene3DRoot = scene3DRoot
        // console.log("Quiz _loadQuiz, scene3DRoot=", scene3DRoot)

        this.scene2DRoot = scene2DRoot

        //// 要回傳給 arController 的 Promise (需要在loadSceneObjects把這個加到 pAll)
        let pQuizCheckRecords = []
        
        if( ! this.quizData.find(q=>q.generalAttr.obj_id==jsonObj.generalAttr.obj_id) ){
        ...
        if(jsonObj.typeAttr.module.trigger.length != 0 ){
            ...
            if( ! this.triggers.find(t => t==trigger_id) ){
                this.triggers.push(trigger_id)
            }

            if( ! this.quizIdWithTriggers.find(q => q.obj_id==jsonObj.generalAttr.obj_id) ){
                this.quizIdWithTriggers.push({ 
                    "obj_id": jsonObj.generalAttr.obj_id,
        ...
        let startQuizFunc = () => {
            if(scene3DRoot!=aQuizAR.scene3DRoot){
                // console.log("%c quiz _loadQuiz _startQuizFunc 但目前不是這個場景 scene3DRoot==aQuizAR.scene3DRoot", "color:wheat", scene3DRoot==aQuizAR.scene3DRoot )
                return;
            }

            this.startQuizFunc = startQuizFunc
            ...
            if(jsonObj.typeAttr.module.trigger.length == 0){    
                ...
                quiz.load( scene3DRoot, scene2DRoot, jsonObj, transformData );
                startQuiz.style.display = "none";
                // QuizStartButton.removeEventListener("click", startQuizFunc)
        ...
        let quitQuizFunc = () => {
            ...
            //// 如果quiz在編輯器的設定是"直接開啟"
            if(jsonObj.typeAttr.module.trigger.length == 0){                
                const openDirectlyData = this.quizIdOpenDirectly.find( data => data.obj_id == jsonObj.generalAttr.obj_id && data.scene3DRoot == this.scene3DRoot )
                if(openDirectlyData){
                    openDirectlyData.clicked = true
                }
            } else {
                let q_t_data = this.quizIdWithTriggers.find( data => data.obj_id == jsonObj.generalAttr.obj_id && data.scene3DRoot == this.scene3DRoot )
                if(q_t_data){
                    q_t_data.clicked = true
                }
            }
        ...
        if( force_login && allow_retry &&  showDirectly && login_id ){
            ...
            this.quizIdOpenDirectly.push( {"obj_id":jsonObj.generalAttr.obj_id, "scene3DRoot":scene3DRoot, "textContext":QuizStartContent.textContent, "allowPlay":false} )
            ...
            let q_t_data = window.aQuizAR.quizIdWithTriggers.find(q=> q.obj_id == jsonObj.generalAttr.obj_id)
            if(q_t_data){
                q_t_data.textContext = QuizStartContent.textContent
                q_t_data.scene3DRoot = scene3DRoot
                q_t_data.allowPlay = true
            }
            ...
            QuizStartButton.addEventListener("click",startQuizFunc);
            //// 記錄有quiz是直接顯示，以便在所有scene_objs都遍歷之後，給定 html UI的 (startQuiz) 的文字  
            this.quizIdOpenDirectly.push( {"obj_id": jsonObj.generalAttr.obj_id, "scene3DRoot":scene3DRoot, "textContext": QuizStartContent.textContent, "allowPlay": true } )
        ...
        function _userNotLoggedIn(showDirectly){
            //// 沒有登入，不給遊玩，跳出提示
            ...
            if( showDirectly ){
                //// 記錄有quiz是直接顯示，以便在所有scene_objs都遍歷之後，給定 html UI的 (startQuiz) 的文字               
                window.aQuizAR.quizIdOpenDirectly.push( {"obj_id": jsonObj.generalAttr.obj_id, "scene3DRoot":scene3DRoot, "textContext": QuizStartContent.textContent, "allowPlay": false } )
            } else {
                let q_t_data = window.aQuizAR.quizIdWithTriggers.find(q=> q.obj_id == jsonObj.generalAttr.obj_id)
                if(q_t_data){
                    q_t_data.textContext = QuizStartContent.textContent
                    q_t_data.scene3DRoot = scene3DRoot
                    q_t_data.allowPlay = false
                }
            }
        }
        ...
        function _quizCheckApiRecord(login_id, proj_id, showDirectly){
            return net.getRecordModule( login_id, proj_id ).then(ret => {
                console.log("arController.js: _getRecordModule: login_id=", login_id, "proj_id=", proj_id , "\n  ret= " , ret );
                ...
                if( showDirectly ){
                    //// 記錄有quiz是直接顯示，以便在所有scene_objs都遍歷之後，給定 html UI的 (startQuiz) 的文字  
                    window.aQuizAR.quizIdOpenDirectly.push({"obj_id": jsonObj.generalAttr.obj_id, "scene3DRoot":scene3DRoot, "textContext": QuizStartContent.textContent, "allowPlay": allowPlay })
                } else {
                    //// 記錄有quiz是點擊物件啟動
                    let q_t_data = window.aQuizAR.quizIdWithTriggers.find(q=> q.obj_id == jsonObj.generalAttr.obj_id)
                    if(q_t_data){
        ...
        function _quizCheckMdbRecord(proj_id, showDirectly){
            return parent.mdb.getProject(proj_id, getEvent => {
                if ( getEvent.target.readyState == "done" && getEvent.target.result ){
                    ...
                    // if( getEvent.target.result.quiz_obj_ids ){
                    if( getEvent.target.result.quizzes ){
                        ...
                        if( getEvent.target.result.quizzes[obj_id] ){
                            ...
                            console.log("arController.js: indexedDB.js _getProject: already have the project with same proj_id and the quiz have been played.", getEvent.target.result , getEvent.target.result.getAwardIndex );
                            ...
                            if( showDirectly ){
                                //// 記錄有quiz是直接顯示，以便在所有scene_objs都遍歷之後，給定 html UI的 (startQuiz) 的文字  
                                window.aQuizAR.quizIdOpenDirectly.push({"obj_id": jsonObj.generalAttr.obj_id, "scene3DRoot":scene3DRoot, "textContext": QuizStartContent.textContent, "allowPlay": true })
                            } else {
                                let q_t_data = window.aQuizAR.quizIdWithTriggers.find(q=> q.obj_id == jsonObj.generalAttr.obj_id)
                                if(q_t_data){
                                    q_t_data.textContext = QuizStartContent.textContent
    ...
    //// for multiple _quizzes:  if there exist any Quiz set with open directly, show the UI (startQuiz).
    //// 掃完辨識圖，場景載入 pAll.then 時，檢查是否有「直接開啟的quiz」
    checkIfQuizOpenDirectly(startQuiz){
        //// 若以後 html ui 改變，就用上面參數傳給它即可
        if( !startQuiz ){
            startQuiz = document.getElementById("startQuiz");
        }

        //// 如果當前沒有任何quiz正在遊玩
        if( !this.checkIsAnyQuizPlaying() ){
            //// 如果存在有「直接開啟的quiz」 顯示 html ui
            if( this.quizIdOpenDirectly.length >= 1 ){	

                //// 當前場景 在loadQuiz時記錄的 直接開啟quiz 的資料
                //// 目前一個場景只能有一個 "直接開啟的quiz"  因此可以用 scene3DRoot 來辨別
                const openDirectlyData = this.quizIdOpenDirectly.find( data => data.scene3DRoot == this.scene3DRoot )
                if( !openDirectlyData ) return;

                QuizStartContent.textContent = openDirectlyData.textContext

                //// 檢查該「直接開啟的quiz」是否已遊玩過  
                const quiz = this.getQuiz(openDirectlyData.obj_id) 
                if( quiz ){
                    //// 檢查該「直接開啟的quiz」是否已遊玩過： 由 該quiz本身記錄的isPlayed 來判斷  (在本次run time已玩過)
                    if( quiz.isPlayed ) startQuiz.style.display = "none";
                    else {
                        startQuiz.style.display = "block";
                        // console.log("%c 這裡?", "color: red; font-size:30px")
                    }

                } else if( openDirectlyData.textContext=="此登入用戶已經遊玩過" && openDirectlyData.allowPlay == false ){

                    if( !openDirectlyData.clicked ){
                        //// 檢查該「直接開啟的quiz」是否已遊玩過： 由 _loadQuiz 依quiz設定 檢查後端和mdb，記錄的isPlayed來判斷  (該quiz設定不可重複遊玩 且已玩過)
                        startQuiz.style.display = "block"
                    ...
                    //// 沒有載入過這個quiz 顯示html ui
                    startQuiz.style.display = "block";
                    // console.log("%c 這裡?", "color: red; font-size:30px")
                    QuizStartButton.addEventListener("click", this.startQuizFunc);
                }
    ...
    checkIfQuizOpenByTrigger(startQuiz, obj_id){
        //// 若以後 html ui 改變，就用上面參數傳給它即可
        if( !startQuiz ){
            startQuiz = document.getElementById("startQuiz");
        }
        ...
        //// 如果當前沒有任何quiz正在遊玩
        if( !this.checkIsAnyQuizPlaying() ){
            //// 如果存在有「點擊開啟的quiz」 顯示 html ui
            if( this.quizIdWithTriggers.length  >= 1 ){	

                //// 當前場景 在loadQuiz時記錄的 點擊開啟quiz 的資料
                //// 目前一個場景只能有一個 "點擊開啟quiz"  因此可以用 scene3DRoot 來辨別
                const q_t_data = this.quizIdWithTriggers.find( data => data.obj_id == obj_id && data.scene3DRoot == this.scene3DRoot )
                if( !q_t_data ) return;

                QuizStartContent.textContent = q_t_data.textContext

                //// 檢查該「點擊開啟quiz」是否已遊玩過  
                const quiz = this.getQuiz(q_t_data.obj_id) 
                if( quiz ){
                    //// 檢查該「點擊開啟quiz」是否已遊玩過： 由 該quiz本身記錄的isPlayed 來判斷  (在本次run time已玩過)
                    if( quiz.isPlayed ) startQuiz.style.display = "none";
                    else {
                        startQuiz.style.display = "block";
                        // console.log("%c 這裡?", "color: red; font-size:30px")
                ...
                } else if( q_t_data.textContext=="此登入用戶已經遊玩過" && q_t_data.allowPlay == false ){
                    // console.log("這裡 q_t_data", q_t_data)
                    if( !q_t_data.clicked ){
                        //// 檢查該「點擊開啟quiz」是否已遊玩過： 由 _loadQuiz 依quiz設定 檢查後端和mdb，記錄的isPlayed來判斷  (該quiz設定不可重複遊玩 且已玩過)
                        startQuiz.style.display = "block"
...
window.aQuizAR = new QuizFactory();
