	...
	////// load the sky, 360 image/video
						...
						case "empty":
							...
							switch (scene_objs[i].sub_type){
								case "quiz":
									...
									//[start-20230706-howardhsu-add]//							
									//// 如果 Quiz 有設定啟動物件
									if(scene_objs[i].module[0].trigger){								
										////  在 Quiz 的啟動物件，增加"TriggerQuiz"屬性，用來記錄啟動哪一個 Quiz 物件																	
			...
			this.loadLight = function( obj, position, rotation, scale ){
				...
				let pLight = new Promise( function( lightResolve ){
					...
					attr = "type:"+obj.light.light_type
			...
			this.loadQuiz = function( obj, position, rotation, scale){
				...
				self.module = {"json":obj.module[0], "quizEntity":Quiz, "currentIndex":0, "score":0, "choices":[], "correctAnswer":0, "totalActiveScoreQuestion":totalActiveScoreQuestion, "record":new Array(obj.module[0].question_list.length), "timer":{"currentTimer":null,"counter":firstTimer } , "record_time":0 , "qClock": Date.now() }
				...
				if ( first_question.options_json ){
					...
					for(let i=0; i<first_question.options_json.length; i++){
						...
						console.log("VRFunc.js: _loadQuiz: _loadOption: ", i , type , first_question.options_json[i] )
						let Entity;
						let pOption;
						switch(type){
							case "txt":
				...
				self.setTransform(Quiz, position, rotation, scale);
...
//20200723-thonsha-add-start
			this.pushButton = function(target){
				...
				let currectQuestion = self.module.json.question_list[idx];
				let scoreType = self.module.json.score_type;
				...
				if (currectQuestion.option_type == "Option_Text" || currectQuestion.option_type == "Option_Image"){
					...
					if (correctAnswer && correctAnswer.obj_id == targetId){
						...
						console.log("VRFunc.js: _nextQuestion: Congratulations! Correct Answer!");
					}
					else{
						answer_is = false;
						makarDragon.style.display = "block";
						...
						console.log("VRFunc.js: _nextQuestion: 叭叭~ Incorrect Answer!");
					}
					window.clearInterval(self.module.timer.currentTimer);
					// vrController.cursorEntity.setAttribute('cursor', "fuse: false; fuseTimeout: 5" );
					// vrController.cursorEntity.setAttribute('animation__mouseenter', "property: geometry.thetaLength; delay: 5; startEvents: mouseenter; dur: 5; from: 0; to: 0" );
					// vrController.cursorEntity.setAttribute('animation__mouseleave', "property: geometry.thetaLength; startEvents: mouseleave; dur: 100; from: 0; to: 0" );
					vrController.cursorEntity.removeAttribute('cursor');
					...
					console.log("VRFunc.js: _nextQuestion: score: ", self.module.score , currectQuestion );
					// goNext = true;
				}
				else{
					...
					if (target.object3D.sub_type == "button"){
						...
						window.clearInterval(self.module.timer.currentTimer);
						// vrController.cursorEntity.setAttribute('cursor', "fuse: false; fuseTimeout: 3000;" );
						// vrController.cursorEntity.setAttribute('animation__mouseenter', "property: geometry.thetaLength; delay: 5; startEvents: mouseenter; dur: 5; from: 0; to: 0" );
						// vrController.cursorEntity.setAttribute('animation__mouseleave', "property: geometry.thetaLength; startEvents: mouseleave; dur: 100; from: 0; to: 0" );
						// vrController.cursorEntity.removeAttribute('cursor');
						...
						setTimeout(function(){
							...
							makarDragon.style.display = "none";
							// 完成前一題，預定紀錄於本地的 database，但是viewer 端並沒有如此操作，之後再說
							// console.log("VRFunc.js: score , currectQuestion = ", idx, self.module , self.module["choices"] , answer_options , get_score );
							console.log("VRFunc.js: _pushButton: " , self.module.json.question_list[idx].time_limit, counter, Math.round( (Date.now() - self.module.qClock)/1000 ) );
							let quizIndex = {
								question: idx ,
								get_score:  get_score,
								answer_time:  Math.round( (Date.now() - self.module.qClock)/1000 ) ,
						...
						console.log("VRFunc.js: score: ", self.module.score , currectQuestion );
						// goNext = true;
					}
				}
...
//[end---20200915- fei 0101-add]//
					...
					if (touchObject.main_type == "button"){
						console.log("VRFunc.js: button click " , touchObject);
						vrController.pushButton(touchObject.el);
					...
					//[start-20230704-howardhsu-add]//
					if (touchObject.triggerQuiz && touchObject.triggerQuiz.played == false){
						//// 顯示問答初始頁面 "點擊開始遊玩"
						let startQuiz = document.getElementById("startQuiz")				
						startQuiz.style.display = "block"
						touchObject.triggerQuiz.played = true	
						
						objectWithClickEvent = true;

						//// 紀錄觸發到的事件
						eventTriggered['3d_quizTriggered'] = touchObject ;
					}
					//[end-20230704-howardhsu-add]//
